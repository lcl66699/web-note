 ...
 * ```
 */
function createRenderer(options) { // --------------coordinate02，coordinate03
...
// implementation
function baseCreateRenderer(options, createHydrationFns) { // --------------coordinate03，coordinate04
    {
    ...
    const patch = (n1, n2, container, anchor = null, parentComponent = null, parentSuspense = null, isSVG = false, slotScopeIds = null, optimized = isHmrUpdating ? false : !!n2.dynamicChildren) => {
      // --------------coordinate04，coordinate05 这里是 patch 声明，即更新节点的核心逻辑
        if (n1 === n2) {
            return;
        }
        ...
        switch (type) {
            case Text: // --------------coordinate05，coordinate06 文本节点的更新，最简单的一个分支
            ...
            case Fragment:
                ...
                break;
            default:
                if (shapeFlag & 1 /* ELEMENT */) { // --------------coordinate08，coordinate09，以此为例
    ...
    const processText = (n1, n2, container, anchor) => {
        if (n1 == null) { // --------------coordinate06，coordinate07，插入
    ...
    const processElement = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => { // --------------coordinate09，coordinate10
        isSVG = isSVG || n2.type === 'svg';
        if (n1 == null) { // 初始化
            mountElement(n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized);
        }
        else { // --------------coordinate10，coordinate11 更新打补丁
    ...
    const patchElement = (n1, n2, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
      // --------------coordinate11，coordinate12，前两个为先后
      // 的虚拟节点参数，格外关注，其他参数先忽略
        const el = (n2.el = n1.el);
        ...
        if (dynamicChildren) {// --------------coordinate18，coordinate19
            patchBlockChildren(n1.dynamicChildren, dynamicChildren, el, parentComponent, parentSuspense, areChildrenSVG, slotScopeIds);
            if (parentComponent && parentComponent.type.__hmrId) {
                traverseStaticChildren(n1, n2);
            }
        }
        else if (!optimized) { // --------------coordinate19，coordinate20
            // full diff
            patchChildren(n1, n2, el, null, parentComponent, parentSuspense, areChildrenSVG, slotScopeIds, false);
        }
        if (patchFlag > 0) { // --------------coordinate12，coordinate13
            ...
            if (patchFlag & 16 /* FULL_PROPS */) {
                ...
                // 依旧是对于 hostPatchProp 的封装，多个属性的更新对比
                // --------------coordinate13，coordinate14
                patchProps(el, n2, oldProps, newProps, parentComponent, parentSuspense, isSVG);
            }
            else { // --------------coordinate17，coordinate18
                // class
    ...
    // --------------coordinate14，coordinate15，内部还是在调用 hostPatchProp
    // coordinate16，coordinate17
    ...
    const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => { // coordinate1
        const componentUpdateFn = () => {
            if (!instance.isMounted) {
                ...
                const nextTree = renderComponentRoot(instance); // coordinate1
                {
                    endMeasure(instance, `render`);
                }
    ...
    const patchChildren = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized = false) => { // --------------coordinate20，coordinate21
        ...
        if (patchFlag > 0) {
            if (patchFlag & 128 /* KEYED_FRAGMENT */) {
                ...
                // --------------coordinate20，coordinate21，diff 算法来了！
...
const nodeOps = { // --------------coordinate07，coordinate08 vdom 到真实 DOM 的转换
...
function ensureRenderer() {
    return (renderer ||
        (renderer = createRenderer(rendererOptions))); // -------------------coordinate01，coordinate02
}
...
const createApp = ((...args) => { // -------------------coordinate00, coordinate01
...
var runtimeDom = /*#__PURE__*/Object.freeze({ // --------------coordinate2，coordinate3
  __proto__: null,
  render: render,
  hydrate: hydrate,
  createApp: createApp,
...
function compileToFunction(template, options) {
    ...
    // the wildcard object.
    const render = (new Function('Vue', code)(runtimeDom)); // --------------coordinate01，coordinate02
    render._rc = true;
