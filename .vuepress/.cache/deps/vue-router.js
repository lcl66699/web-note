...
// node_modules/vue-router/node_modules/@vue/reactivity/dist/reactivity.esm-bundler.js
...
// node_modules/vue-router/node_modules/@vue/runtime-core/dist/runtime-core.esm-bundler.js
var stack = [];
...
// node_modules/vue-router/node_modules/@vue/runtime-dom/dist/runtime-dom.esm-bundler.js
...
// node_modules/vue-router/node_modules/vue/dist/vue.runtime.esm-bundler.js
function initDev() {
  {
    initCustomFormatter();
  }
...
function resolveRelativePath(to, from) {
  if (to.startsWith("/"))
    return to;
  if (!from.startsWith("/")) {
    warn2(`Cannot resolve a relative location without an absolute path. Trying to resolve "${to}" from "${from}". It should look like "/${from}".`);
    return to;
  }
  if (!to)
    return from;
...
function scrollToPosition(position) {
  ...
  if ("el" in position) {
      ...
      if (!isIdSelector || !document.getElementById(position.el.slice(1))) {
        try {
          ...
          if (isIdSelector && foundEl) {
            warn2(`The selector "${position.el}" should be passed as "el: document.querySelector('${position.el}')" because it starts with "#".`);
            return;
          }
        } catch (err) {
          warn2(`The selector "${position.el}" is invalid. If you are using an id selector, make sure to escape it. You can find more information about escaping characters in selectors at https://mathiasbynens.be/notes/css-escapes or use CSS.escape (https://developer.mozilla.org/en-US/docs/Web/API/CSS/escape).`);
          return;
        }
      }
    }
    ...
    if (!el) {
      warn2(`Couldn't find element using selector "${position.el}" returned by scrollBehavior.`);
      return;
    }
...
function useHistoryStateNavigation(base) {
  ...
  function changeLocation(to, state, replace2) {
    ...
    try {
      ...
      if (true) {
        warn2("Error with push/replace State", err);
      } else {
        console.error(err);
      }
  ...
  function push(to, data) {
    ...
    if (!history2.state) {
      warn2(`history.state seems to have been manually replaced without preserving the necessary values. Make sure to preserve existing history state if you are manually calling history.replaceState:
...
function createWebHashHistory(base) {
  ...
  if (!base.endsWith("#/") && !base.endsWith("#")) {
    warn2(`A hash base must end with a "#":
  ...
  if (true) {
    ...
    for (const key of parser.keys) {
      if (existingKeys.has(key.name))
        warn2(`Found duplicated params with name "${key.name}" for path "${record.path}". Only the last one will be available on "$route.params".`);
...
function createRouterMatcher(routes, globalOptions) {
  ...
  function resolve(location2, currentLocation) {
    ...
    if ("name" in location2 && location2.name) {
      ...
      path = location2.path;
      if (!path.startsWith("/")) {
        warn2(`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a bug in vue-router. Please open an issue at https://new-issue.vuejs.org/?repo=vuejs/router.`);
      }
...
function checkSameParams(a, b) {
  for (const key of a.keys) {
    if (!key.optional && !b.keys.find(isSameParam.bind(null, key)))
      return warn2(`Alias "${b.record.path}" and the original record: "${a.record.path}" should have the exact same param named "${key.name}"`);
  }
...
function checkChildMissingNameWithEmptyPath(mainNormalizedRecord, parent) {
  if (parent && parent.record.name && !mainNormalizedRecord.name && !mainNormalizedRecord.path) {
    warn2(`The route named "${String(parent.record.name)}" has a child without a name and an empty path. Using that name won't render the empty path child so you probably want to move the name to the child instead. If this is intentional, add a name to the child route to remove the warning.`);
  }
}
function checkMissingParamsInAbsolutePath(record, parent) {
  for (const key of parent.keys) {
    if (!record.keys.find(isSameParam.bind(null, key)))
      return warn2(`Absolute path "${record.record.path}" should have the exact same param named "${key.name}" as its parent "${parent.record.path}".`);
  }
}
...
function decode(text) {
  try {
    ...
    warn2(`Error decoding "${text}". Using original value`);
  }
  return "" + text;
}
...
function onBeforeRouteLeave(leaveGuard) {
  if (!getCurrentInstance()) {
    warn2("getCurrentInstance() returned null. onBeforeRouteLeave() must be called at the top of a setup function");
    return;
  }
  ...
  if (!activeRecord) {
    warn2("No active route record was found when calling `onBeforeRouteLeave()`. Make sure you call this function inside of a component child of <router-view>. Maybe you called it inside of App.vue?");
    return;
  }
...
function onBeforeRouteUpdate(updateGuard) {
  if (!getCurrentInstance()) {
    warn2("getCurrentInstance() returned null. onBeforeRouteUpdate() must be called at the top of a setup function");
    return;
  }
  const activeRecord = inject(
    matchedRouteKey,
  ...
  if (!activeRecord) {
    warn2("No active route record was found when calling `onBeforeRouteUpdate()`. Make sure you call this function inside of a component child of <router-view>. Maybe you called it inside of App.vue?");
    return;
  }
...
function canOnlyBeCalledOnce(next, to, from) {
  ...
  return function() {
    if (called++ === 1)
      warn2(`The "next" callback was called more than once in one navigation guard when going from "${from.fullPath}" to "${to.fullPath}". It should be called exactly one time in each navigation guard. This will fail in production.`);
    next._called = true;
    if (called === 1)
...
function extractComponentsGuards(matched, guardType, to, from) {
  ...
  for (const record of matched) {
    if (!record.components && !record.children.length) {
      warn2(`Record with path "${record.path}" is either missing a "component(s)" or "children" property.`);
    }
    for (const name in record.components) {
      let rawComponent = record.components[name];
      if (true) {
        if (!rawComponent || typeof rawComponent !== "object" && typeof rawComponent !== "function") {
          warn2(`Component "${name}" in record with path "${record.path}" is not a valid component. Received "${String(rawComponent)}".`);
          throw new Error("Invalid route component");
        } else if ("then" in rawComponent) {
          warn2(`Component "${name}" in record with path "${record.path}" is a Promise instead of a function that returns a Promise. Did you write "import('./MyPage.vue')" instead of "() => import('./MyPage.vue')" ? This will break in production if not fixed.`);
          const promise = rawComponent;
          rawComponent = () => promise;
        } else if (rawComponent.__asyncLoader && !rawComponent.__warnedDefineAsync) {
          rawComponent.__warnedDefineAsync = true;
          warn2(`Component "${name}" in record with path "${record.path}" is defined using "defineAsyncComponent()". Write "() => import('./MyPage.vue')" instead of "defineAsyncComponent(() => import('./MyPage.vue'))".`);
        }
      }
      ...
      if (isRouteComponent(rawComponent)) {
        ...
        if (!("catch" in componentPromise)) {
          warn2(`Component "${name}" in record with path "${record.path}" is a function that does not return a Promise. If you were passing a functional component, make sure to add a "displayName" to the component. This will break in production if not fixed.`);
...
function createRouter(options) {
  ...
  function removeRoute(name) {
    ...
    if (recordMatcher) {
      ...
      warn2(`Cannot remove non-existent route "${String(name)}"`);
    }
  }
  function getRoutes() {
  ...
  function resolve(rawLocation, currentLocation) {
    ...
    if (typeof rawLocation === "string") {
      ...
      if (true) {
        if (href2.startsWith("//"))
          warn2(`Location "${rawLocation}" resolved to "${href2}". A resolved location cannot start with multiple slashes.`);
        else if (!matchedRoute2.matched.length) {
          warn2(`No match found for location with path "${rawLocation}"`);
        }
      }
    ...
    if ("path" in rawLocation) {
      if ("params" in rawLocation && !("name" in rawLocation) && Object.keys(rawLocation.params).length) {
        warn2(`Path "${rawLocation.path}" was passed with params but they will be ignored. Use a named route alongside params instead.`);
      }
    ...
    if (hash && !hash.startsWith("#")) {
      warn2(`A \`hash\` should always start with the character "#". Replace "${hash}" with "#${hash}".`);
    }
    ...
    if (true) {
      if (href.startsWith("//")) {
        warn2(`Location "${rawLocation}" resolved to "${href}". A resolved location cannot start with multiple slashes.`);
      } else if (!matchedRoute.matched.length) {
        warn2(`No match found for location with path "${"path" in rawLocation ? rawLocation.path : rawLocation}"`);
      }
    }
    return assign({
      fullPath,
 ...
 when navigating to "${to.fullPath}". A redirect must contain a name or path. This will break in production.`);
      ...
      if (failure2) {
        if (isNavigationFailure(failure2, 2)) {
          if (isSameRouteLocation(stringifyQuery$1, resolve(failure2.to), toLocation) && redirectedFrom && (redirectedFrom._count = redirectedFrom._count ? redirectedFrom._count + 1 : 1) > 10) {
            warn2(`Detected an infinite redirection in a navigation guard when going from "${from.fullPath}" to "${toLocation.fullPath}". Aborting to avoid a Stack Overflow. This will break in production if not fixed.`);
  ...
  function triggerError(error, to, from) {
    ...
    if (list.length) {
      ...
      if (true) {
        warn2("uncaught error during route navigation:");
      }
      console.error(error);
    }
    ...
    install(app) {
      ...
      if (isBrowser && !started && currentRoute.value === START_LOCATION_NORMALIZED) {
        ...
        push(routerHistory.location).catch((err) => {
          if (true)
            warn2("Unexpected error when starting the router:", err);
        });
      }
      const reactiveRoute = {};
