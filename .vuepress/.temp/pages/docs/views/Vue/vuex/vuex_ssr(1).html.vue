<template><div><h1 id="vuex-ssr" tabindex="-1"><a class="header-anchor" href="#vuex-ssr" aria-hidden="true">#</a> Vuex &amp; SSR</h1>
<blockquote>
<p>波比 - 资深航空母舰拼接工程师｜一级螺丝工</p>
</blockquote>
<p>课程大纲：</p>
<ul>
<li>关于状态管理的一点思考</li>
...
<p>##关于状态管理的一点思考</p>
<blockquote>
<p>首先抛出一些问题，什么是状态？为什么要管理？怎么去管理？</p>
</blockquote>
<p>我谈一下我的理解</p>
<p>前端发展到今天，更主流的模式是 <code v-pre>UI = f(state)</code>，视图通过底层框架（<code v-pre>vue</code>，<code v-pre>react</code> 等提供的能力）根据<strong>状态</strong>进行驱动。</p>
<p>对前端开发工程师而言，挑战变成了如何更好的去定义、更新、存储、共享这些状态，因为状态在某一时刻的快照与视图一一对应，且非常具有函数式编程范儿的（同样的输入一定有相同的输出，同样的状态一定对应一样的视图展示）。</p>
<blockquote>
<p>状态管理是当前前端最核心的部分，因为视图能用状态描述，所以前端的逻辑复杂度移到了如何让状态“不要出错”，前端与后端的一个区别在于，状态在前端是长生命周期的，又会在这个生命周期中有不可枚举地、未知地、难以追踪的多次修改，保证其按照既定轨迹不出错就很困难了</p>
</blockquote>
<blockquote>
<p>为什么说是某一时刻，状态不是一成不变的，实际业务场景中，状态是一直在被改变的，从而带来视图的随之更新。</p>
</blockquote>
<p>最广义程度来看，状态本质是还是一种可以描述视图状态、行为的数据结构，状态的管理则是通过一定的算法将这些数据结构组织、管理起来，又回到了 <code v-pre>程序=算法+数据结构</code> 这一基本概念。</p>
<p>往细了看</p>
<p>状态大致可以分为两类，<strong>本地状态</strong>和<strong>共享状态</strong></p>
<p>本地状态就是 <code v-pre>vue</code> 中的 <code v-pre>data</code>，<code v-pre>react</code>中的 <code v-pre>state</code>，这里我们一般会用来控制弹窗的现实隐藏、<code v-pre>loading</code>效果等</p>
<p>共享状态其实是最头疼的问题，但又是最常见的场景，业务中肯定会出现大量需要兄弟节点通信、祖孙节点通信等情况的场景，通信的目的是为了状态分享，虽然可以通过一些方式，比如回调函数等手段实现，但都不是最佳实践</p>
<blockquote>
<p>一般情况下，你不需要什么状态管理工具，简单是避免麻烦的最佳实践，当你的应用膨胀到你已经无法理顺状态流的时候，才是你考虑使用状态管理工具的时机</p>
<p>项目的复杂度和组件层次结构的复杂性是衡量是否使用状态管理工具的标准，这多多少少取决于经验</p>
</blockquote>
<p>面临的问题</p>
<p>状态管理的方式：中心化和去中心化两种模式</p>
<p><code v-pre>Dan</code> 曾经说过 <code v-pre>SPA</code> 应用最佳实践是分为容器组件和展示组件，通过 <code v-pre>props</code> 向下传递状态，这样分层之后的好处显而易见，只需要维护好容器组件内的状态就行，展示组件和业务逻辑解耦，但是真的是这样吗？分析公司内的项目代码，大家可能会发现，经常出现层级嵌套个5-6层的页面，这么传下去谁也不敢保证中间环节不出什么问题，还是做不到单一，解耦。</p>
<p>所以 <code v-pre>Flux</code> 架构及其追随者 <code v-pre>Redux</code> <code v-pre>Vuex</code>被提出，主要思想是<strong>应用的状态被集中存放到一个仓库中，但是仓库中的状态不能被直接修改</strong>，<strong>必须通过特定的方式</strong>才能更新状态</p>
<p>这样，当我们完全控制了状态的改变，我们可以很容易的追踪状态的变化也让应用开发更容易调试。使用开发者工具我们不仅可以检查状态的变化，甚至可以回到上个状态（类似时间旅行）</p>
<blockquote>
<p>但是这种模式下写的代码真的很恶心</p>
</blockquote>
<p>状态划分</p>
<p>状态划分的粒度可大可小，<code v-pre>react</code> 社区最新的状态管理工具<a href="https://github.com/facebookexperimental/Recoil" target="_blank" rel="noopener noreferrer"><code v-pre>recoil</code><ExternalLinkIcon/></a> 从原子粒度 <code v-pre>atom</code> 来划分。</p>
<p>对我们而言，什么方式才是最好的呢？</p>
<p>有团队区分业务型和视觉模型，直白的说就是如视图控制的状态归一，业务逻辑相关的归一，看似美好且清爽，在实际业务实践下仍然有很大的难度，区分好模型完全靠经验，状态分的太多了共享，复用也是问题</p>
<p>我目前比较倾向于领域模型的方式，可参考<a href="https://tech.meituan.com/2017/12/22/ddd-in-practice.html" target="_blank" rel="noopener noreferrer">DDD 美团实践<ExternalLinkIcon/></a></p>
<p>理想的状态管理工具需要解决的问题</p>
<ol>
<li>状态更新的设计，<code v-pre>API</code> 足够少，且简单</li>
<li>如何共享状态</li>
<li>状态提升</li>
<li>状态下降</li>
<li>同步、异步的处理</li>
...
<p>总的来说，这仍然是一个有很大空间的方向</p>
<h2 id="vuex-原理" tabindex="-1"><a class="header-anchor" href="#vuex-原理" aria-hidden="true">#</a> Vuex 原理</h2>
<blockquote>
<p>因为课程长度原因，且实践部分大多比较基础，VUEX 的实践部分就不做探究了，给大家找了一个实践代码项目，一本小册，看完基本从基础到高阶都可以掌握，看完之后搭配着原理分析更香</p>
...
<h4 id="vuex-核心原理分析" tabindex="-1"><a class="header-anchor" href="#vuex-核心原理分析" aria-hidden="true">#</a> vuex 核心原理分析</h4>
<p><a href="https://github.com/vuejs/vuex/tree/dev/src" target="_blank" rel="noopener noreferrer">源代码地址<ExternalLinkIcon/></a></p>
<p>我们不直接硬生生的去分析源码，现在希望大家抛掉对 <code v-pre>vuex</code> 的所有记忆，让我们回到几年前<code v-pre>vuex</code>诞生的那个时间点，从头开始去思考基于 <code v-pre>FLUX</code> 思想，如何打造一个我们自己的状态管理工具</p>
<p>a. 站在<code v-pre>FLUX</code> 的角度去思考</p>
<p>在开发中面临最多的场景是状态重复但是不集中</p>
<div class="language-vue ext-vue line-numbers-mode"><pre v-pre class="language-vue"><code>// a.vue
...
*/
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里其实就出现了状态重复的问题，在不同的组件中依赖了同样的状态，重复就会导致不对等的风险，基于 <code v-pre>FLUX</code> 的思想，我们设计的状态管理将是中心化的工具，也就是集中式存储管理应用的所有组件的状态，大白话，就是将所有的状态放在一个全局的 <code v-pre>Tree</code> 结构中，集中放在一起的好处是可以有效避免重复的问题，也更好的管理，将状态和视图层解耦</p>
<blockquote>
<p>TJ 原来说过我的状态管理就是 {}，对中心化的管理工具来说，不就是这样嘛😂</p>
</blockquote>
<p>b. 如何去更改状态</p>
<p>自由即代表了混乱，<code v-pre>FLUX</code> 推崇以一种可预测的方式发生变化，而且有且唯一一种，这样的好处是所有的行为可预测，可测试，对于之后做个<code v-pre> dev-tool</code> 去调试、时间旅行都很方便，现在的问题就是要去思考同步和异步的问题了，为了区分的更清楚，我们定义两种行为，<code v-pre>Actions</code> 用来处理异步状态变更（内部还是调用 <code v-pre>Mutations</code>），<code v-pre>Mutations</code> 处理同步的状态变更，整个链路应该是一个闭环，单向的，完美契合 <code v-pre>FLUX</code> 的思想</p>
<p>「页面 dispatch/commit」-&gt; 「actions/mutations」-&gt; 「状态变更」-&gt; 「页面更新」-&gt; 「页面 dispatch/commit」...</p>
<p>现在，可以借用这个图了</p>
<p><img src="https://i1.wp.com/user-gold-cdn.xitu.io/2020/4/2/171396e1d42df794?w=701&amp;h=551&amp;f=png&amp;s=30808" alt="vuex"></p>
<p>c. 如何和 <code v-pre>vue</code> 集成呢？</p>
<p>插件呀～这样可以和 <code v-pre>vue</code> 集成在一起，通过 <code v-pre>mixin</code> 将 <code v-pre>$store</code> 这样的快速访问 <code v-pre>store</code> 的快捷属性注入到每一个 <code v-pre>vue</code> 实例中</p>
<blockquote>
<p><a href="https://cn.vuejs.org/v2/guide/plugins.html" target="_blank" rel="noopener noreferrer">开发 vue 插件<ExternalLinkIcon/></a></p>
</blockquote>
<p>d. 怎么让 <code v-pre>store</code> 是响应式的呢？</p>
<p>利用 <code v-pre>vue</code> <code v-pre>data</code> 里的状态是响应式的啊～</p>
<p>e. 开始撸码</p>
<p>Step1 - store 注册</p>
...
<span class="token keyword">export</span> <span class="token keyword">class</span> <span class="token class-name">Store</span> <span class="token punctuation">{</span>
  <span class="token function">constructor</span><span class="token punctuation">(</span><span class="token parameter">options <span class="token operator">=</span> <span class="token punctuation">{</span><span class="token punctuation">}</span></span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 简单处理一下，衍生不就是计算一下嘛，传人 state</span>
...
<span class="token keyword">function</span> <span class="token function">resetStoreVM</span><span class="token punctuation">(</span><span class="token parameter">store<span class="token punctuation">,</span> state</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Step4 - Actions/Mutations</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token doc-comment comment">/**
...
<span class="token keyword">function</span> <span class="token function">resetStoreVM</span><span class="token punctuation">(</span><span class="token parameter">store<span class="token punctuation">,</span> state</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Step5 - 分形，拆分出多个 Module</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>以上只是以最简化的代码实现了 <code v-pre>vuex</code> 核心的 <code v-pre>state</code> <code v-pre>module</code> <code v-pre>actions</code> <code v-pre>mutations</code> <code v-pre>getters</code> 机制，如果对源代码感兴趣，可以看看<a href="https://juejin.cn/post/6844904001192853511#heading-12" target="_blank" rel="noopener noreferrer">若川的文章<ExternalLinkIcon/></a></p>
...
<p>过程如下：</p>
<ol>
<li>浏览器通过请求得到一个<code v-pre>HTML</code>文本</li>
<li>渲染进程解析<code v-pre>HTML</code>文本，构建<code v-pre>DOM</code>树</li>
<li>解析<code v-pre>HTML</code>的同时，如果遇到内联样式或者样式脚本，则下载并构建样式规则（<code v-pre>stytle rules</code>），若遇到<code v-pre>JavaScrip</code>t脚本，则会下载执行脚本。</li>
<li><code v-pre>DOM</code>树和样式规则构建完成之后，渲染进程将两者合并成渲染树（<code v-pre>render tree</code>）</li>
<li>渲染进程开始对渲染树进行布局，生成布局树（<code v-pre>layout tree</code>）</li>
<li>渲染进程对布局树进行绘制，生成绘制记录</li>
<li>渲染进程的对布局树进行分层，分别栅格化每一层，并得到合成帧</li>
<li>渲染进程将合成帧信息发送给<code v-pre>GPU</code>进程显示到页面中</li>
</ol>
<p>很容易发现，<code v-pre>CSR</code>  的特点就是会在浏览器端的运行时去动态的渲染、更新 <code v-pre>DOM</code> 节点，特别是 <code v-pre>SPA</code> 应用来说，其模版 <code v-pre>HTML</code> 只有一个 <code v-pre>DIV</code>，然后是运行时（<code v-pre>React</code>，<code v-pre>Vue</code>，<code v-pre>Svelte</code> 等）动态的往里插入内容，这样的话各种 <code v-pre>BaiduSpider</code> 拿不到啥有效信息，自然 <code v-pre>SEO</code> 就不好了，项目一旦复杂起来， <code v-pre>bundle</code> 可能超乎寻常的大...这也是一个开销</p>
<p>那么<code v-pre>SSR</code> 呢，则是是服务端完成了渲染过程，将渲染完成的 <code v-pre>HTML</code> 字符串或者流返回给浏览器，就少了脚本解析、运行这一环节，理论上 <code v-pre>FP</code> 表现的更佳，<code v-pre>SEO</code> 同样</p>
<p><img src="https://raw.githubusercontent.com/yacan8/blog/master/images/服务端渲染原理/image-20200731165404271.png" alt="csr vs ssr"></p>
<p>但其实，现在 <code v-pre>SSR</code> 也并没有大行其道，凡事有利有弊，<code v-pre>SSR</code> 也是有缺点的</p>
<ol>
<li>复杂，同构项目的代码复杂度直线上升，因为要兼容两种环境</li>
<li>对服务端的开销大，既然 <code v-pre>HTML</code> 都是拼接好的，那么传输的数据肯定就大多了，同时，拿 <code v-pre>Node</code> 举例，在处理 <code v-pre>Computed</code> 密集型逻辑的时候是阻塞的，不得不上负载均衡、缓存策略等来提升</li>
<li>CI/CD 更麻烦了，需要在一个 <code v-pre>Server</code> 环境，比如 <code v-pre>Node</code></li>
</ol>
<blockquote>
<p>一般来说，ToB 的业务场景基本不需要 SSR，需要 SSR 的一定是对首屏或者 SEO 有强诉求的，不然没必要搞那么麻烦，简洁是避免麻烦的最佳实践，同时，随着浏览器发展，越来越快，爬虫也越来越智能，SSR 的场景在被压缩</p>
</blockquote>
<p>彩蛋，这里说到了 <code v-pre>CSR</code> 和 <code v-pre>SSR</code> ，其实我们现今常见的渲染方案有6-7种吧！</p>
<p><img src="https://image-static.segmentfault.com/324/269/3242695953-5c7c0095b3cf5_fix732" alt="render"></p>
<p>注意，这里提到了 <code v-pre>hydration</code> 这个词，这是一个很棒的思路，对 <code v-pre>FP</code> 有帮助，但是不能提升 <code v-pre>TTI</code></p>
<blockquote>
<p>补充资料，务必精读</p>
...
<p>我们以上面的指南为基础讲讲同构应用（因为同构应用算是比较复杂的了），通过同构应用让大家对 <code v-pre>SSR</code> 有一个更直观、立体的认识</p>
<p>首先需要了解什么是同构应用</p>
<blockquote>
...
<p>假定大家已经认真阅读并实际操练了 <code v-pre>VUE SSR</code> 指南，</p>
<p>现在就一些实践经验做一些补充：</p>
<ol>
<li>
<p>服务端的 <code v-pre>webpack</code> 不用关注 <code v-pre>CSS</code>，客户端会打包出来的，到时候推 <code v-pre>CDN</code>，然后改一下 <code v-pre>public path</code> 就好了</p>
</li>
<li>
<p>服务端的代码不需要分 <code v-pre>chunk</code>，<code v-pre>Node</code> 基于内存一次性读取反而更高效</p>
</li>
<li>
<p>如果有一些方法需要在特定的环境执行，比如客户端环境中上报日志，可以利用 <code v-pre>beforeMouted</code> 之后的生命周期都不会在服务端执行这一特点，当然也可以使用 <code v-pre>isBrowser</code> 这种判断</p>
</li>
<li>
<p><code v-pre>CSR</code> 和 <code v-pre>SSR</code> 的切换和降级</p>
...
<li>
<p><code v-pre>Axios</code> 封装，至少区分环境，在客户端环境是需要做代理的</p>
</li>
</ol>
<blockquote>
<p>这里的最佳实践知识抛砖引玉，还是得自己去踩坑总结</p>
</blockquote>
<p><code v-pre>VUE-SSR</code> 优化方案：</p>
<ol>
<li>页面级别的缓存，比如 <code v-pre>nginx</code> <code v-pre>micro-caching</code></li>
<li>设置 <code v-pre>serverCacheKey</code>，如果相同，将使用缓存，组件级别的缓存</li>
<li><code v-pre>CGI</code> 缓存，通过 <code v-pre>memcache</code>  等，将相同的数据返回缓存一下，注意设置缓存更新机制</li>
<li>流式传输，但是必须在<code v-pre> asyncData</code> 之后，否则没有数据，说明也可能会被 <code v-pre>CGI</code> 耗时阻塞</li>
<li>分块传输，这样前置的 <code v-pre>CGI</code> 完成就会渲染输出，但是这个方案难啊</li>
<li><a href="https://juejin.cn/post/6844903476120518670" target="_blank" rel="noopener noreferrer">JSC<ExternalLinkIcon/></a>，就是不用 <code v-pre>vue-loader</code></li>
</ol>
</div></template>
